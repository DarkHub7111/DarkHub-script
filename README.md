...
-- ========== BUY TAB ==========  
local function AddBuyToggle(key, title, remoteName)  
    Tabs.Buy:AddToggle(key, { Title = title, Default = false }):OnChanged(function(state)  
        getgenv()[key] = state  
        if state then  
            task.spawn(function()  
                while getgenv()[key] do  
                    pcall(function()  
                        if NinjaEvent and typeof(NinjaEvent.FireServer) == "function" then  
                            NinjaEvent:FireServer(remoteName, "Blazing Vortex Island")  
                        else  
                            -- fallback: try rEvents if ninjaEvent not found  
                            if ReplicatedStorage:FindFirstChild("rEvents") and ReplicatedStorage.rEvents:FindFirstChild(remoteName) then  
                                ReplicatedStorage.rEvents[remoteName]:FireServer("Blazing Vortex Island")  
                            else  
                                -- some servers use unified "buyAllSwords" etc on ninjaEvent only  
                                pcall(function()  
                                    if ReplicatedStorage:FindFirstChild("rEvents") then  
                                        -- try generic FireServer on rEvents  
                                        ReplicatedStorage.rEvents:FireServer(remoteName, "Blazing Vortex Island")  
                                    end  
                                end)  
                            end  
                        end  
...
AddBuyToggle("BuyAllSwords", "Buy All Swords", "buyAllSwords")  
AddBuyToggle("BuyAllBelts", "Buy All Belts", "buyAllBelts")  
AddBuyToggle("BuyAllSkills", "Buy All Skills", "buyAllSkills")  
